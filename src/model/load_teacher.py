...
def load_backbone_and_classifier(backbone_path, classifier_path, name, num_classes, map_location="cpu"):
    ...
    # backbone(FC 제외) 로드
    bb = torch.load(backbone_path, map_location=map_location, weights_only=True)
    m.load_state_dict({k: v for k, v in bb.items() if not k.startswith("fc.")}, strict=False)
    # classifier 로드
    cls = torch.load(classifier_path, map_location=map_location, weights_only=True)
...
def load_student_imagenet(resnet18_pt_path, num_classes, map_location="cpu"):
    ...
    sd = torch.load(resnet18_pt_path, map_location=map_location, weights_only=True)
